...
const handleError = (
  ...
  if (error instanceof ErrorBase) {
    ...
    res.status(responsecodeStatus).json(new InternalServerError(get(error, 'message'), undefined, responsecodeStatus, get(error, 'type'), get(error, 'additionalProperties')));
  }
};
